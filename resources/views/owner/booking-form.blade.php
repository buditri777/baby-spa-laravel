...
<div class="page-header">
  <h4>{{ isset($booking) ? 'Edit Booking' : 'Buat Booking' }}</h4>
  <p class="page-sub">{{ isset($booking) ? 'Perbarui data booking.' : 'Buat booking sesi baru.' }}</p>
</div>
    ...
    <div class="row g-3">
      <div class="col-md-6">
        ...
        <select name="child_id" class="form-select" required>
          ...
          @foreach($children ?? [] as $c)
            <option value="{{ $c->id }}" @selected(isset($booking) && $booking->child_id===$c->id)>
      ...
      <div class="col-md-6">
        ...
        <select name="service_id" class="form-select" required>
          ...
          @foreach($services ?? [] as $s)
            <option value="{{ $s->id }}" @selected(isset($booking) && $booking->service_id===$s->id)>
      ...
      <div class="col-md-6">
        ...
        <select name="therapist_id" class="form-select">
          ...
          @foreach($therapists ?? [] as $t)
            <option value="{{ $t->id }}" @selected(isset($booking) && $booking->therapist_id===$t->id)>
              {{ $t->name }}
            </option>
          @endforeach
        </select>
      ...
      <div class="col-md-6">
        ...
        <select name="branch_id" class="form-select" required>
          ...
          @foreach($branches ?? [] as $br)
            <option value="{{ $br->id }}" @selected(isset($booking) && $booking->branch_id===$br->id)>
              {{ $br->name }}
            </option>
          @endforeach
        </select>
      ...
      <div class="col-md-6">
        ...
        <input type="date" name="scheduled_date" class="form-control"
               value="{{ isset($booking) ? $booking->scheduled_date?->format('Y-m-d') : '' }}" required>
      </div>
      <div class="col-md-6">
        <label class="form-label">Jam</label>
        <input type="time" name="scheduled_time" class="form-control"
               value="{{ $booking->scheduled_time ?? '' }}" required>
      </div>
      @if(isset($booking))
      <div class="col-md-6">
      ...
      <div class="col-12">
        <div class="form-check form-switch">
          <input class="form-check-input" type="checkbox" name="is_homecare" id="isHomecare"
                 value="1" @checked(isset($booking) && $booking->is_homecare)>
