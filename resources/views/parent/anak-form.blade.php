...
<div class="page-header">
  <h4>{{ isset($child) ? "Edit Data Anak" : "Tambah Data Anak" }}</h4>
  <p class="page-sub">{{ isset($child) ? "Perbarui informasi si kecil." : "Daftarkan anak baru." }}</p>
</div>
    ...
    <div class="row g-3">
      ...
      <div class="col-md-6">
        ...
        <input type="date" name="birth_date" class="form-control"
               value="{{ isset($child) ? $child->birth_date?->format('Y-m-d') : '' }}" required>
      </div>
      <div class="col-12">
        <label class="form-label">Alergi</label>
        <textarea name="allergies" class="form-control" rows="2">{{ $child->allergies ?? '' }}</textarea>
      </div>
      <div class="col-12">
        <label class="form-label">Kondisi Medis</label>
        <textarea name="medical_conditions" class="form-control" rows="2">{{ $child->medical_conditions ?? '' }}</textarea>
      </div>
    </div>
